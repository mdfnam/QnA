...
- **Suffixes:** Certain suffixes can indicate abstract nouns. For example, words ending in `ism` (capitalism, socialism), `ity` (equality, purity), `ment` (development, improvement), `tion` or `sion` (communication, decision) and `ing` (swimming, cooking) are often associated with abstract concepts.
